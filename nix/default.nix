...
let
  ...
  devPackageNames = [ "utop" "ocamlformat" ];
  devPackages =
    builtins.map (name: builtins.getAttr name opam-selection) devPackageNames;

  resolve = opam2nix.resolve (args // {
    selection = "./nix/opam-selection_${ocamlVersion}.nix";
  }) (localPackages ++ devPackageNames);
...
in (builtins.listToAttrs (builtins.map (fname:
  ...
  in {
    ...
    inherit resolve opam-selection devPackages;
  }
